...
const chatsContentsRepository: IChatsContentsRepository = {
    ...
    async insertMessagesChatContent(chat_id: number, ...messagesDto: IMessageDto[]) {
        try {
            const messages = JSON.stringify(messagesDto).replace(/(\[|\])/g, '');
            const subQuery = 'select json_agg(msg_element.elements) from (select elements from chats_contents, jsonb_array_elements(messages) as elements where chat_id = ? union select ?::jsonb) as msg_element';
